...
export default function DelchanDashboard() {
  const router = useRouter();
  const { setCurrentUser, updateDashboardState } = useAppStore();
  const workouts = useUserWorkouts('Delchan');
  const personalRecords = useUserPersonalRecords('Delchan');

  useEffect(() => {
    setCurrentUser('Delchan');
    updateDashboardState({ selectedUser: 'Delchan' });
  ...
  const handleWorkoutTypeClick = (workoutType: WorkoutType) => {
    ...
    router.push(`/delchan/workout-types/${workoutType}`);
  };
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <LiquidButton
            icon={<Plus size={16} />}
            onClick={() => router.push('/delchan/workouts/add')}
          >
            Add Workout
          </LiquidButton>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <LiquidButton
              variant="outline"
              size="sm"
              onClick={() => router.push('/delchan/workouts')}
            >
              View All
            </LiquidButton>
          </div>
          ...
          {recentWorkouts.length > 0 ? (
            ...
            <GlassCard className="p-12 text-center">
              <div className="text-gray-500 dark:text-gray-400">
                ...
                <LiquidButton onClick={() => router.push('/delchan/workouts/add')}>
